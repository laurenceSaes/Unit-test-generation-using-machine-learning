#!/bin/bash
fromProjects=9999;
inputSizes=(10000 25000 50000);
modes=(normal mixed sbt)
compresions=(0 2 5 10)
database="unittests_all"
outputDir="Data"
...
renameForGoogle () {
   ...
   if [ -f "$location/compress.dict.tab" ]
   then
      cp "$location/compress.dict.bypass.tab" "$train/vocab.sources.txt"
      cp "$location/compress.dict.bypass.tab" "$train/vocab.targets.txt"
   fi
   ...
   then
	...
	mv $train/targets.txt.bpe $trainBPE/targets.txt
        ...
        cp $location/compress.dict.bpe $trainBPE/vocab.sources.txt
        mv $location/compress.dict.bpe $trainBPE/vocab.targets.txt
...
do
	...
	do
		...
		do
			...
			renameForGoogle "$compressLocation" "$compressTimes"
		done
